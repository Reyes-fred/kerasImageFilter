...
# Definition of the loss and helper functions
...
def calculate_loss(gImArr):
    #Calculate total loss with K.function
...
def save_original_size(x, target_size=baseImageSizeOrig):
...
cModel = VGG16(include_top=False, weights='imagenet', input_tensor=baseImArr)
sModel = VGG16(include_top=False, weights='imagenet', input_tensor=filterImArr)
...
P = get_feature_reps(x=baseImArr, layer_names=[cLayerName], model=cModel)[0]
As = get_feature_reps(x=filterImArr, layer_names=sLayerNames, model=sModel)
